...
contract DSCEngineTest is Test {
    ...
    function testCanDepositCollateralAndGetAccountInfo() public depositCollateral {
        (uint256 totoDscMinted, uint256 collateralValueInUsd) = dsce.getAccountInfo(USER);

        uint256 expectedTotalDscMinted = 0;
        uint256 expectedDepositAmount = dsce.getTokenAmountFromUsd(weth, collateralValueInUsd);
        assertEq(totoDscMinted, expectedTotalDscMinted);
    ...
    function testEmitCollateralRedeemedWithCorrectArgs() public depositCollateral {
        ...
        emit DSCEngine.CollateralRedeemed(USER, USER, AMOUNT_COLLATERAL, weth);
        vm.startPrank(USER);
        dsce.redeemCollateral(weth, AMOUNT_COLLATERAL);
        vm.stopPrank();
    }
